...
file = [20180322, 071405]; %Cs and Na
file = [20180321, 233402]; %long Cs
file = [20180322, 120956]; % after spectra
file = [20180407, 184453]; % after spectra
...
% Define start points
%             a    b      c    w  (MHz)
StartNaL = {[0.36, 18.51, 0, 0.012], [0.36, 18.145, 0, 0.012], [0.36, 18.125, 0, 0.012]};
StartNaR = {[0.36, 18.68, 0, 0.012], [0.36, 19.116, 0, 0.012], [0.36, 19.125, 0, 0.012]};
...
for i = 1 : 3
    ...
    % Fit
    if bFit
        ftL = fitData(x/scale, y, 'a*exp(-(x-b)^2/w^2) + 0*c', ...
            'Start', StartNaL{i}, 'FitCenter', FitCenterNaL{i}, 'Plot', 1);
        ftR = fitData(x/scale, y, 'a*exp(-(x-b)^2/w^2) + 0*c', ...
...
% Define start points
%             a    b  c  w  (kHz)
StartCsL = {[0.6, 19, 0, 20], [0.6, -115, 0, 13], [0.6, -114, 0, 13]};
StartCsR = {[0.6, 70, 0, 7], [0.6, 140, 0, 10], [0.6, 152, 0, 10]};

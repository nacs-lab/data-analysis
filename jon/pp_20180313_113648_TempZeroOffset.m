...
file = [20180311, 003821];
file = [20180312, 222019];
file = [20180313, 113648];
...
StartNaL = {[0.36, 18.51, 0.012], [0.36, 18.135, 0.012], [0.36, 18.11, 0.012]};
StartNaR = {[0.36, 18.68, 0.012], [0.36, 19.12, 0.012], [0.36, 19.11, 0.012]};
...
for i = 1 : 3
    ...
    % Fit
    if bFit
        ftL = fitData(x/scale, y, 'a*exp(-(x-b)^2/w^2)', ...
            'Start', StartNaL{i}, 'FitCenter', FitCenterNaL{i}, 'Plot', 1);
        ftR = fitData(x/scale, y, 'a*exp(-(x-b)^2/w^2)', ...
...
%% Plot Cs
% Define start points
% StartCsL = {[0.6, 15, 0, 20], [0.6, -127, 0, 13], [0.6, -127, 0, 13]};
% StartCsR = {[0.6, 65, 0, 7], [0.6, 145, 0, 10], [0.6, 145, 0, 10]};
StartCsL = {[0.6, 20,  20], [0.6, -148,  13], [0.6, -148,  13]};
StartCsR = {[0.6, 65+10,  7], [0.6, 145+10,  10], [0.6, 145+10,  10]};
